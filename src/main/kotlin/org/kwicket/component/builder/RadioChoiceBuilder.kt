...
interface IRadioChoiceBuilder<T> : IComponentBuilder<RadioChoice<T>, T> {
...
class RadioChoiceBuilder<T>(
...
) : IRadioChoiceBuilder<T>,
    ComponentBuilder<RadioChoice<T>, T>(
        model = model,
        markupId = markupId,
    ...
    override fun build(id: String): RadioChoice<T> =
        radioChoiceFactory(
            id = id,
            model = model,
            choices = choices,
