...
fun KClass<*>.cssResRef(name: String, deps: List<HeaderItem>? = null): CssResourceReference =
    if (deps.isNullOrEmpty()) CssResourceReference(java, name)
    else object : CssResourceReference(java, name) {
        override fun getDependencies(): List<HeaderItem> = deps.toList()
    }

fun KClass<*>.jsResRef(name: String, deps: List<HeaderItem>?? = null): JavaScriptResourceReference =
    if (deps.isNullOrEmpty()) JavaScriptResourceReference(java, name)
    else object : JavaScriptResourceReference(java, name) {
        override fun getDependencies(): List<HeaderItem> = deps.toList()
    }

fun KClass<*>.cssHeaderItem(name: String, deps: List<HeaderItem>? = null): CssReferenceHeaderItem =
    CssHeaderItem.forReference(cssResRef(name = name, deps = deps))

fun KClass<*>.jsHeaderItem(name: String, deps: List<HeaderItem>? = null): JavaScriptReferenceHeaderItem =
