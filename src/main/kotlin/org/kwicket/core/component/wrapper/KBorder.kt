 ...
 *      element, creating a placeholder wicketId if the component is initially not isVisible
 * @param visible optional flag indicating whether the [Component] is isVisible
 * @param visibilityAllowed sets whether visibility is allowed on this [Panel] if the parameter is not null
 * @param enabled optional flag indicating whether the [Component] is isEnabled
...
abstract class KBorder(
    ...
    var onConfig: (Border.() -> Unit)? = null,
    block: (Border.() -> Unit)? = null
) : Border(id, model) {
